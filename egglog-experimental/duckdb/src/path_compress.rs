...
//! concrete types, and stable primitive names.  Generated rule, function,
...
pub(crate) fn compile_path_compression(
    ...
    let RuleBodyCall::Primitive { name, output, .. } = &inequality.head else {
        bail!(
        ...
        name.as_ref() == "!=" && scalar(base_values, *output)? == ScalarSqlType::Unit,
        "DuckDB path rule `{}` requires primitive `!= (Id, Id) -> Unit`",
        rule.name
    );
    ...
    let neq_result = var(neq_result).ok_or_else(|| {
        ...
        scalar(base_values, neq_result.ty)? == ScalarSqlType::Unit,
    ...
    let (fresh, head_fresh_label) = head_fresh(base_values, &rule.name, fresh_action)?;
    ensure!(
...
fn head_fresh<'a>(
    ...
    let RuleActionCall::Primitive { name, output, .. } = call else {
        ...
        "DuckDB path rule `{rule_name}` requires get-fresh! (String) -> Id"
    );
...
fn validate_union_find_merge(
    ...
    let sym_fresh_label = expect_fresh_merge(base_values, rule_name, fresh_sym, 2)?;
    ...
    let trans_fresh_label = expect_fresh_merge(base_values, rule_name, fresh_trans, 3)?;
    ensure!(sym_fresh_label == trans_fresh_label);
...
fn expect_let_primitive(
    ...
    expect_primitive(base_values, rule_name, value, name, arguments, input_count)
}
...
fn expect_fresh_merge(
    ...
    ensure!(scalar(base_values, *ty)? == ScalarSqlType::String);
    Ok(RuleValue {
        value: *value,
        ty: *ty,
    })
...
fn expect_primitive(
    ...
    let MergeFn::Primitive {
        ...
        bail!("DuckDB path rule `{rule_name}` merge requires primitive `{expected_name}`");
    };
    ensure!(
        name == expected_name
            && input.len() == expected_inputs
            && input.iter().all(|ty| *ty == ColumnTy::Id)
            && *output == ColumnTy::Id
            && args.len() == expected_args.len(),
        "DuckDB path rule `{rule_name}` primitive `{expected_name}` has an incompatible typed signature"
    );
