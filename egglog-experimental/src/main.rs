...
fn main() {
    ...
    let egraph = if proof_mode {
        ...
        egglog_experimental::new_experimental_egraph_with_disequality_encoding(
            disequality_encoding.unwrap_or_default(),
        )
    };
...
fn validate_disequality_mode(
    encoding: Option<DisequalityEncoding>,
    args: &[OsString],
) -> Result<bool, String> {
    ...
    if proof_mode && encoding.is_some() {
        return Err("--disequality-encoding is currently supported only in normal mode; it cannot be combined with --term-encoding, --proofs, --proof-testing, or --proof-extraction".to_owned());
    }
    Ok(proof_mode)
}
...
) -> Result<(Option<DisequalityEncoding>, Vec<OsString>), String> {
...
mod tests {
    ...
    fn parses_and_filters_disequality_encoding() {
        ...
        .unwrap();
        assert_eq!(encoding, Some(DisequalityEncoding::DisequalityEdges));
    ...
    #[test]
    fn rejects_an_explicit_encoding_in_every_term_and_proof_mode() {
        for flag in [
            "--term-encoding",
            "--proofs",
            "--proof-testing",
            ...
            assert!(
                validate_disequality_mode(Some(DisequalityEncoding::EqualityEmbedding), &args)
                    .unwrap_err()
                    .contains("supported only in normal mode")
            );
        }
        assert!(
            !validate_disequality_mode(None, &[OsString::from("egglog-experimental")]).unwrap()
        );
    }
    ...
    fn mode_flags_after_end_of_options_are_input_paths() {
        for flag in [
            ...
            assert!(!validate_disequality_mode(None, &args).unwrap());
        }
    }
}
