...
fn main() {
    ...
    let egraph = match backend {
        ...
        Backend::FlowLog => egglog_experimental::new_experimental_egraph_with_backend_for_proofs(
            Box::new(egglog_experimental_flowlog::EGraph::new_interpret()),
        ),
    };
        ...
        match arg.to_str() {
            Some("--backend") => {
                ...
                let Some(value) = iter.next() else {
                    return Err("--backend requires one of: main, flowlog".to_string());
                };
...
fn parse_backend(value: Option<&str>) -> Result<Backend, String> {
    match value {
        ...
        Some("flowlog") => Ok(Backend::FlowLog),
        Some(other) => Err(format!(
            "unknown backend {other:?}; expected one of: main, flowlog"
        )),
