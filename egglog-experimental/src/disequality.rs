...
impl DisequalityMacro {
    fn transform_fail_action_run(
        &self,
        actions: Vec<Action>,
        ...
        if contains_disequality(&actions) {
            self.transform(Command::Actions(actions), symbol_gen, type_info)
        } else {
            let mut transformed = Vec::with_capacity(actions.0.len());
            for action in actions.0 {
                transformed.extend(self.transform(
    ...
    fn transform(
        ...
        match command {
            ...
            Command::Fail(span, commands) => {
                // A source-level `fail` may contain adjacent action commands whose
                // bindings are visible to later actions. Lower those actions as one
                // batch so `lower_actions` can type the whole sequence before the
                // core engine executes it transactionally.
                let mut transformed = Vec::new();
                let mut pending_actions = Vec::new();
                for command in commands {
                    match command {
                        Command::Action(action) => pending_actions.push(action),
                        command => {
                            if !pending_actions.is_empty() {
                                transformed.extend(self.transform_fail_action_run(
                                    std::mem::take(&mut pending_actions),
                                    symbol_gen,
                                    type_info,
                                )?);
                            }
                            transformed.extend(self.transform(command, symbol_gen, type_info)?);
                        }
                    }
                }
                if !pending_actions.is_empty() {
...
mod tests {
    use crate::{
        DisequalityEncoding, new_experimental_egraph,
        new_experimental_egraph_with_disequality_encoding,
    };
    use egglog::{Error, TypeError, ast::sanitize_internal_names};
    ...
    #[test]
    fn all_encodings_expand_fail_children_in_normal_mode() {
        let programs = [
            r#"
                (datatype Math (A) (B))
                (fail
                  ...
                  (disequal $x (B))
                  (check (= (A) (B))))
                ; The failed transaction must not leave its disequality behind.
                (fail (check-known-disequal (A) (B)))
            "#,
            r#"
                (datatype Math (A) (B))
                (disequal (A) (B))
        ...
        for encoding in ENCODINGS {
            for program in programs {
                ...
                egraph
                    .parse_and_run_program(None, program)
                    .unwrap_or_else(|error| {
                        panic!("{encoding:?} failed source-order fail expansion: {error}")
                    });
            }
        }
    }
    ...
    #[test]
    fn disequality_macro_does_not_group_unrelated_fail_actions() {
        let program = r#"
            (datatype Math (A))
            (fail (let $x (A)) (panic "stop"))
        "#;

        for encoding in ENCODINGS {
            let error = new_experimental_egraph_with_disequality_encoding(encoding)
                .parse_and_run_program(None, program)
                .unwrap_err();
            assert!(
                error
                    .to_string()
                    .contains("definitions, user-defined commands, push, and pop are not allowed"),
                "{encoding:?} changed unrelated fail-action scoping: {error}"
            );
        }
    }

    #[test]
    fn disequality_macro_preserves_explicit_begin_scope_inside_fail() {
        let program = r#"
            (datatype Math (A) (B))
            (fail
              (begin
        ...
        for encoding in ENCODINGS {
            let error = new_experimental_egraph_with_disequality_encoding(encoding)
                ...
                .unwrap_err();
            assert!(
                matches!(error, Error::TypeError(TypeError::Unbound(ref name, _)) if name == "$local"),
                "{encoding:?} leaked an explicit begin binding into a neighboring action: {error}"
            );
        }
    }
    ...
    fn disequality_adjacency_sets_merge_keys_and_rebuild_neighbors() {
        ...
        new_experimental_egraph_with_disequality_encoding(DisequalityEncoding::DisequalityEdges)
            .parse_and_run_program(None, program)
            .unwrap_or_else(|error| panic!("DE failed adjacency rebuild: {error}"));
    }

    #[test]
    fn all_disequality_fixtures_run_with_every_encoding() {
        for fixture in disequality_fixtures() {
            ...
            for encoding in ENCODINGS {
                ...
                egraph
                    ...
                    .unwrap_or_else(|error| {
                        panic!("{encoding:?} failed {}: {error}", fixture.display())
                    });
                if parameter_facts.is_some() {
                    egraph
                        ...
                        .unwrap_or_else(|error| {
                            panic!(
                                "{encoding:?} lost reconstructed terms in {}: {error}",
                                fixture.display()
                            )
                        });
                }
        ...
        for fixture in fixtures {
            ...
            for encoding in ENCODINGS {
                ...
                replay
                    ...
                    .unwrap_or_else(|error| {
                        panic!(
                            "{encoding:?} snapshot for {} failed to replay: {error}",
                            fixture.display()
                        )
                    });
                if parameter_facts.is_some() {
