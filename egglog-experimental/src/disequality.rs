    ...
    fn disequality_fixture_desugared_snapshots_match_and_replay() {
        ...
        for fixture in fixtures {
            ...
            for encoding in ENCODINGS {
                ...
                let mut modes = vec![("ordinary", egglog::EGraph::default())];
                if encoding != DisequalityEncoding::DisequalityEdges {
                    modes.extend([
                        ("term", egglog::EGraph::new_with_term_encoding()),
                        ("proofs", egglog::EGraph::new_with_proofs()),
                        (
                            "proof-testing",
                            egglog::EGraph::new_with_proofs().with_proof_testing(),
                        ),
                    ]);
                    #[cfg(feature = "bin")]
                    modes.push((
                        "proof-extraction",
                        egglog::EGraph::default().with_proof_extraction(),
                    ));
                }
                for (mode, mut replay) in modes {
                    if let Some(facts) = &parameter_facts {
