...
| `nee` | Negated equality embedding | A private `ne(x, y)` relation and a self-loop check |
...
Disequality compilation is currently a normal-mode experiment. The CLI rejects
an explicit `--disequality-encoding` together with `--term-encoding`,
`--proofs`, `--proof-testing`, or `--proof-extraction`. Programs that do not
select disequality retain the existing term/proof behavior.

The [integration guide](../benchmarks/disequality/EGGLOG_INTEGRATION.md)
documents the paper examples, generated snapshots, host integrations, and
benchmark commands.
