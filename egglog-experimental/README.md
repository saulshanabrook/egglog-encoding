...
DE is currently supported only in ordinary execution. The CLI rejects `de`
when combined with `--term-encoding`, `--proofs`, `--proof-testing`, or
`--proof-extraction`; the proof-oriented Rust constructor rejects it as well.
EE, OEE, and NEE remain covered in all five modes. DE uses literal egglog
containers, and proof/term encoding does not yet support the required
set-valued custom merge. Rust callers should not bypass this boundary by
enabling a proof mode after constructing a normal DE e-graph; the hidden core
mode-enabling path enforces the same guard.
