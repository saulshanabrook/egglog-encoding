"""Numeric analysis of benchmark rows: row selection, means with confidence
intervals, and target/baseline ratios via Fieller intervals. Pure computation —
no table model, no formatting. Shared by report.py and, via Pyodide, the
eval-live consumer; pandera is only touched lazily in selected_rows(validate=True).
"""
