...
#[test]
fn traced_external_with_fallback_records_only_primary_successes() {
    let mut db = Database::default();
    let [source, sink] = (0..2)
        .map(|_| {
            db.add_table(
...

    {
        ...
        for value in [1, 2, 3, 5] {
            buffer.stage_insert(&[v(value), v(0)]);
        }
    }
    db.merge_all();
    ...
    let increment = db.add_external_function(Box::new(make_external_func(|_, args| {
        let [value] = args else { panic!() };
        (value.rep() != 5).then(|| value.inc())
    })));
    ...
    let mut rule = query.build();
    let result = rule
        .call_external_with_fallback(assert_even, &[value.into()], increment, &[value.into()])
        .unwrap();
    rule.insert(sink, &[result.into(), payload.into()]).unwrap();
    rule.build_with_description("trace-primary-primitive");
    ...
    assert_eq!(primitive.args, vec![v(2)]);
    assert_eq!(primitive.result, v(2));
    assert_eq!(
        trace.matches[primitive.origin.index()].bindings,
        vec![("value".into(), v(2)), ("payload".into(), v(0))]
    );
