...
//! | Stored positions | [`HistoryPosition`] uniquely orders retained fact, equality, rekey, removal, and check events. A firing stores a sampled inclusive high-water cutoff rather than a unique event position. | Historical lookup, liveness, strict pre-event closure, and chronological replay scheduling. | Waves cannot order maintenance events within one unit. Treating firing cutoffs as unique events would instead invent an order between same-batch matches. |
...
//! | Facts | [`FactId`] addresses one arena record; [`RawFactRecord`] retains its table, creation position, exact [`CauseRef`], and immutable raw row. [`FactCellRef`] selects one typed occurrence. | [`TraceView::fact`], [`TraceView::fact_cell_at`], and backward premise traversal. | Premises, constructor occurrences, historical values, and structural origins must refer to the exact row that existed, not a later equal row. |
...
//! | Applied equalities | [`RawAppliedEquality`] retains the typed raw proposal endpoints, actual native `child -> parent` forest edge, event position, and [`EqualityReason`]. [`ProjectedAppliedEquality`] adds only the lazily recovered structural [`EqualityEndpoint`]s. | [`TraceView::applied_equality`], [`TraceView::project_applied_equality`], equality explanation, and cause traversal. | Proposal syntax names what the program equated; the native edge makes historical connectivity unambiguous; the reason identifies the program action, merge, or maintenance cause that must be selected. None can be reconstructed from the other two. |
//! | Structural replay terms | [`ReplayTermId`] identifies interned literal/call [`ReplayTerm`] nodes; [`ReplayConstructorSpec`] and per-row/per-rule origin recipes connect raw values to structural syntax. | Lazy internal fact projection, [`TraceView::replay_term`], [`TraceView::firing_terms`], and command lowering. | Raw native values are opaque and their representative denotation can change under canonicalization; replay needs stable, typed syntax, but projecting every term on the capture path would make tracing expensive. |
//! | Core replay schemas | [`ReplayTableSchema`], binding/equality recipes, table key/kind metadata, constructors, merge origins, and container sorts describe how physical rows encode logical syntax. | [`TraceView::table_schema`], structural projection, interference selection, and command lowering. | Dynamic events alone cannot recover typed columns, keys, constructor structure, or merge-cell provenance. |
