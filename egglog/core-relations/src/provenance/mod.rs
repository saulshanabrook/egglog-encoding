...
//! | Firings and premises | [`Firing`] retains rule, wave, sampled [`Firing::history_cutoff`], premise [`FactId`]s, and reads performed by merge callbacks. Static binding recipes and [`FiringEqualitySource`] layouts describe how source-order bindings and guards map back to those premises. | [`TraceView::firing`], [`TraceView::firing_terms`], and rule replay. | A selected head action is reproducible only with its exact grounded match and equality state. Occurrence-index rule bodies fail closed because a table-row [`FactId`] does not retain the index key or `any_of` witness metadata. |
//! | Merge callback reads | [`MergeRead::Fact`] retains an exact prior row and callback cutoff. [`MergeRead::Constructor`] retains the consulted constructor table, the structural `old`/`new` origin, observed native key, cutoff, and optional committed hit. Sparse read lists attach to their firing, source bundle, or non-rule cause. | [`Firing::merge_reads`], [`TraceView::source_merge_reads`], [`TraceView::cause_merge_reads`], [`TraceView::explain_merge_constructor_read`], and backward closure. | A merge callback is an ordinary egglog expression and can read state beyond its two scalar inputs. A ground-looking constructor call still reads the callback-time canonicalization environment: replaying the same syntax without that denotation can turn a hit into a miss, while retaining a same-batch prediction's first creator would be unnecessarily imprecise. |
//! | Causes | [`CauseRef`] tags either a firing or a shared [`RawCause`] node; non-rule nodes retain source identity, prior facts, merge ancestry, or rebuild/container landmarks. A [`RawCause::Merge`] also stores the inclusive `history_cutoff` at which its callback read both rows, while sparse cause-level merge reads retain additional committed rows consulted by nested callbacks. | [`TraceView::cause`], [`TraceView::cause_merge_reads`], and lazy backward slicing. Merge endpoint denotation uses the callback cutoff, while the later [`RawAppliedEquality::position`] validates the applied native edge. | Shared causal nodes avoid copying dependency prefixes while preserving the exact reason an effective fact or equality appeared. Separating read time from application time prevents a replacement fact or deferred equality from explaining an earlier callback read. |
