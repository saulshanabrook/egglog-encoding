...
//! |---|---|---|---|
//! | Temporal coordinates | [`Wave`] groups effects that share a pre-wave state. [`HistoryPosition`] uniquely orders retained fact, equality, rekey, removal, and check events; a firing stores a sampled inclusive high-water cutoff rather than a unique position. [`EdgeHorizon`] counts the applied-equality prefix visible at an observation. | Historical lookup, equality explanation, liveness, and replay scheduling. | A wave cannot order events within maintenance, a history cutoff alone does not identify the equality prefix, and treating firing cutoffs as events would invent an order between same-batch matches. |
...
//! | Criteria | [`Criterion`] retains the first successful check's wave and event position, premises, typed endpoint pairs, their [`CriterionEndpointOccurrence`]s, and equality cutoff. | [`TraceView::check_root`], [`TraceView::check_roots`], slice-root selection, and replay check scheduling. | The slicer needs the exact observed witness and historical state; rerunning a query could choose another witness, and scheduling the check at another cutoff could expose a different row lifetime or equality relation. |
