...
pub struct FunctionConfig {
    ...
    pub n_vals: usize,
    /// Opt-in identity-prefix guard. An exact repeat of the complete value tuple always keeps the
    /// existing row without running the merge program. In addition, `Some(k)` marks the first `k`
    /// value columns (`k` in `1..=n_vals`) as *identity*: when a key collision leaves that prefix
    /// unchanged, the merge is likewise skipped (a subsume-flag change still applies). `None` (the
    /// default) disables only this broader prefix guard. Only opt in for merges whose payload is
    /// irrelevant when their identity prefix is unchanged.
    ...
    /// The ordered actions and result expressions used to resolve FD conflicts.
...
impl EGraph {
    ...
    pub fn try_add_table(&mut self, config: FunctionConfig) -> Result<FunctionId> {
        ...
        // tuple-output function whose merge has the wrong number of results can't silently
        // mis-split its key and value columns. `check_references` cross-checks the result count,
        // explicit value-column references, and merge-local binding order.
        assert!(
        ...
        merge.check_references(n_vals, &name);
        ...
        // whose `:merge` does a `set` into its OWN table): merge resolution (`fill_deps` /
...
struct RuleInfo {
    ...
    grounded_rule: Option<GroundedRuleInfo>,
    desc: Arc<str>,
}

#[derive(Clone)]
struct GroundedRuleInfo {
    rule: Arc<core_relations::GroundedRule>,
    variables: DenseIdMap<VariableId, core_relations::Variable>,
}
