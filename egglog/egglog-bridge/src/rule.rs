...
impl Query {
    ...
    pub(crate) fn supports_grounded_execution(&self) -> bool {
        self.capture.is_none() && self.atoms.iter().all(|atom| atom.occurrence.is_none())
    }
        ...
        Ok(match &self.capture {
            ...
            Some(QueryCapture::Criterion(capture)) => {
                let endpoint = |source: CriterionCapturePremise| {
                    let atom = self.atoms.get(source.premise).unwrap_or_else(|| {
                        panic!(
                    ...
                    assert!(
                        source.column < atom.schema.func_cols,
                        "criterion capture endpoint premise {} column {} cites an engine-only column",
                        source.premise,
                        source.column
                    );
                    let entry = atom.entries.get(source.column).unwrap_or_else(|| {
                        panic!(
    ...
    pub(crate) fn build_grounded_rule(
        ...
        anyhow::ensure!(
            ...
            "rules with trace capture or occurrence-index queries cannot use grounded execution"
        );
        let mut rsb = RuleSetBuilder::new(db);
        let (mut qb, mut inner) = self.query_state(&mut rsb);
        ...
        for atom in &self.atoms {
            let vars = inner.convert_all(&atom.entries);
            let _ = match &atom.occurrence {
                Some((entry, cols)) => {
                    qb.add_occurrence_atom(atom.table, &vars, inner.convert(entry), cols, &[])?
                }
                None => qb.add_atom(atom.table, &vars, &[])?,
            };
        }
        let mut rb = qb.build();
        inner.next_ts = Some(rb.read_counter(self.ts_counter).into());
        ...
        let probes = self
            ...
            .map(|atom| {
                core_relations::GroundedProbe::new(
                    atom.table,
                    inner.convert_all(&atom.entries).into_vec(),
                    atom.schema.n_keys,
                )
            })
            .collect::<Vec<_>>();
