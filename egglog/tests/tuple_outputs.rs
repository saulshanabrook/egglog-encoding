...
use egglog::{ApiError, EGraph, Error, Read, TypeError, Write};
...
#[test]
fn single_output_rust_apis_reject_tuple_functions() {
    ...
    egraph
        .parse_and_run_program(
            ...
            (set (iv 1) (values 10 20))
            "#,
        )
        .unwrap();

    for err in [
        egraph.function_entries("iv", |_| {}).unwrap_err(),
        egraph
            .update(|state| state.lookup("iv", 1_i64))
            .unwrap_err(),
        egraph
            .update(|mut state| state.set("iv", 1_i64, 30_i64))
            .unwrap_err(),
    ] {
        assert!(matches!(
            err,
            Error::ApiError(ApiError::TupleOutputUnsupported { ref name, .. }) if name == "iv"
        ));
    }
}
