...
- In the term/proof encoding, a term-mode constructor's `get-fresh!` now receives its constructor name and children — the same key `set-if-empty` interns on — so a backend may content-address the mint and return one id per `(constructor, children)` term instead of minting a fresh candidate that `set-if-empty` discards on every hit. The default backend runtime still ignores the extra arguments and mints fresh, so this is behavior-preserving; it enables content-addressed (stable-under-re-derivation) minting.
