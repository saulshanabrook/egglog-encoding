...
- **Every eq-sort global of a sort shares one table.** `remove_globals` gave each global its own nullary function, so the term/proof encoding gave each one a term relation, an FD view, a rebuild index and a rebuild rule; 1000 global `let`s expanded to 9049 encoded commands and 3009 installed rules, against 53 and 10 for 1000 local `let`s in one rule. A global is now one row of `@Globals_<Sort>`, keyed by a slot id, so that schema is emitted once per sort. The table is named by the fresh generator, so no program can declare a function that collides with it — and because it is reserved, a `let` no longer counts towards `(get-size!)`, matching `print-size`, which already skipped globals. Doing it in the desugaring rather than in the encoder means the term encoding collapses the same way. Base-sort globals keep their own nullary function: they never go stale, so they had no view or rebuild rule to share. On `luminal-llama` under `--proofs` this is 8.09 s to 6.38 s wall. Proof output is unchanged (every proof snapshot is byte-identical); proof extraction skips a global's row so a proof still names the term the global was bound to, and serialization and `(print-function <global>)` both read the global's own name back from the row's key rather than showing the shared table.
