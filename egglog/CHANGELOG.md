...
- **Term encoding rebuilds via the correlated `or`.** Without proofs, the per-constructor rebuild rule now uses the correlated, deduplicating disjunction: one hidden per-sort staleness relation (maintained from non-canonical UF parent edges) with one `OR` branch per eq-sort column, so a row stale in several columns still rebuilds exactly once (delta-driven). A view with a single eq-sort column degenerates to the plain delta-driven rule. Results are unchanged (same fixpoint); proof mode keeps the previous guarded-`or` rule since `or` is unsupported under proofs. See `docs/disjunction-design.md`.
