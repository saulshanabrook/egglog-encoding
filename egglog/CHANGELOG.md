...
- `(fail <command>+)` now accepts multiple commands, running them in order and succeeding if any one fails (previously it wrapped a single command). Desugaring, global removal, and proof encoding keep the whole expansion of a wrapped command inside the `fail`, so `fail` now works over commands that expand to several — including `(fail (set …))` under the term/proof encoding. Live children are macro-expanded once in source order, so a later child can use a declaration or global binding from the successful prefix. Proof testing checks a generated proof inside `fail` against only the command prefix that reached it, so preceding actions can justify the proof while later or unreachable rules cannot. Each failing source command rolls back its database, compiler, scope, and proof state as one unit while the successful prefix remains committed; constructor rows created while evaluating that failing command no longer leak. Static desugaring rejects a `fail` body that may change compiler state in ways resolved output cannot represent, because which changes survive depends on runtime failure order.
