...
- **One union-find function per sort (proof mode).** The proof term encoding now
  represents each sort's union-find as one tuple function `@UF : (S) -> (S, Proof)`
  (parent + a proof `key = parent`), replacing the former two-table
  `@UF_S (S S) -> Proof` relation plus `@UF_Sf` index. Conflicting parents are
  resolved by the function's `:merge`, which keeps the smaller e-class and unions the
  displaced one back into `@UF` (constructor-view congruence works the same way), so
  the `single_parent`/`uf_function_index` rules and per-term self-loop seeds are gone.
  This mirrors the term (non-proof) encoding's `@UF : (S) -> S`, whose merge is now
  spelled out as a `:merge` action block in the generated program.
