...
- In the term/proof encoding, when a rule head rebuilds a constructor application the body already matched, reuse the body view read's e-class instead of minting a fresh id and re-interning it (dropping the redundant `get-fresh!`, term-relation row, and `set-if-empty`). The match is keyed on the instrumented children, so it composes through nested terms. In proof mode the *natural* node is still built (a parent's `Congr` and the rule-head check need its as-built shape), but the view dedup is skipped: the body view read's proof `eclass = f(children)` stands in for the re-read view proof, so the connector `natural = eclass` composes directly from it.
