...
- Fix `file_supports_proofs` wrongly rejecting programs whose `(push)`-scoped globals are read in actions: the whole-program check ran against the final (popped) scope, so scoped globals looked like unsupported function lookups. Six corpus files (herbie, array, bdd, cyk, math, typeinfer) now run under the term/proof encoding tests. Herbie also surfaced that primitive computations in queries were not actually checked: base-value primitive facts (like container-building facts before them) are now *side conditions*, re-evaluated against the rule body with the program's typed validators. Termified base values that appear as proposition leaves (a BigRat variable's `(bigrat (from-string …) …)` — canonical spellings the program never wrote) get a new `Computed` proof form, verified by running the value's sort-declared recognizer (`value_term_validator`, the base-sort analogue of `rebuild_container_normalizer`). `Fiat` is reserved for top-level (global) equalities again, and `from-string` gains a primitive validator.
