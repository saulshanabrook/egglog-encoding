...
- Add check-directed replay slicing. `--slice-output FILE` records
  a supported serial run, emits only the dynamic support cone of successful
  checks, and writes the rendered replay program directly; strict replay is a
  corpus/CI invariant rather than a second production CLI pass. Capture-time
  unsupported boundaries remain fail-closed. `--slice` requests replay and
  composes with normal, proof, proof-testing, term-encoding, proof-extraction,
  naive, and serialization modes; `--slice-output` alone does not replay.
  Retained rewrites and birewrites preserve their source form and receive
  deterministic names for exact grounded replay.
  The Rust CLI surface is intentionally consolidated from `cli(EGraph)` and
  `cli_with_args(EGraph, args)` to
  `cli(EGraph, args, replay_factory)`; embedding binaries must supply the
  factory used to execute a generated slice on a fresh graph. Library callers
  receive owned replay commands from `slicing::slice_all_checks` and can run
  them directly with `EGraph::run_program`; the CLI uses that direct path too,
  while rendering is only the artifact output and round-trip testing path.
...
- **Breaking Rust API:** `egglog_bridge::MergeFn` is split into `MergeProgram` and `MergeExpr`, and
  ...
  it also supports the exact binary constructor result `Constructor(old, new)` by retaining a
  committed constructor hit or attributing a deferred miss. Other table reads, multi-output, and
  actionful merge results remain unsupported. Merge causes record the callback's operand-read cutoff
  separately from the later application of any equality staged by the callback.
