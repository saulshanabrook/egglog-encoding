...
- **Breaking reporting formats.** `--save-report` now stores each iteration with its ruleset name and timing responsibility, and no longer serializes the redundant `ruleset_timings` or `search_and_apply_time_per_rule` aggregates. `--timing-summary` now emits the typed version-5 timing partition used by the benchmark runner, including exclusive construction, signature-catalog, resolution, and generated-lowering counters; the runner's JSONL schema is also version 5, so older disposable benchmark caches must be recomputed.
