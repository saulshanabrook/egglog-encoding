...
- **Fix rule-set time going unrecorded when a user-defined command runs the rules.** The overall run report was written by the `run-schedule` command, so it only saw rule sets that command ran. egglog-experimental replaces `run-schedule` with its own extended scheduler, which runs a `(run <ruleset>)` step by calling `EGraph::step_rules` directly — those iterations reached no report at all. On `luminal-llama` that silently dropped 10 of 16 rule sets, among them the single most expensive one in the run (`matmul_backend`, 45% of wall). Recording now happens in `step_rules` and `step_rules_with_scheduler`, the one place every rule iteration passes through, so it no longer depends on which command drove it. Only the native backend was affected: the term/proof encoding disables the extended scheduler, so its reports were already complete. Both are public API, and an embedder that already unions their returned report into an overall report of its own will now count those iterations twice.
- **The benchmark timing summary accounts for the work outside rule-set execution.** It reported only per-ruleset search/apply/merge/rebuild, so a runner could do no better than subtract those from wall time and call the rest one residual — 74% of it on `luminal-llama` under `--proofs`. The summary (schema version 3) now also carries each ruleset's assembly time — planning a rule and re-deriving its plan for an iteration, which runs before the search timer starts — and the phases that run outside every ruleset: parse, typecheck, desugar, encoding, declare-and-compile, top-level actions, schedule, and proof extraction. A user-defined command counts as a schedule, since one can replace `run-schedule` outright, and is charged only what it spends beyond the rule sets it drives and the commands it runs in turn. The residual keeps its place as `Unattributed`, and is now 1–5% of wall on `luminal-llama` and `eggcc-2mm-pass1` in both native and proof mode.
- Fix a global's shared table outliving, or failing to outlive, the command that declared it. `(fail c1 c2)` stops at the first command that fails, so a table left inside the `fail` could go undeclared while a later global still expected it — global removal now hoists the declaration out. A command rejected after its globals were name-checked kept both its rows and its names, so the name became unbindable and the next global of that sort wrote to a table that was never declared; resolving a command now keeps its reservations only if it resolved. The checks that recognize a global binding were also reading three separate top-level-only scans of the program, one of which the term encoder needed; they are one scan now.
- Fix a desugared program failing to replay under the term/proof encoding. `:internal-global-table` is not surface syntax, so a shared table printed and re-parsed arrives as a plain `:internal-let` function; the encoder recognized it as a global only when the read was nullary, so a keyed one was encoded as building a term. It now recognizes either spelling. Replaying an eq-sort global's program worked before this branch, so this was a regression.
- Fix a `:merge` body that reads a global. The encoder treated the read as building a term, so it wrote the merge's own term relation with the global's slot key where an argument belonged; and the proof checker re-ran the merge body with only `old`/`new` bound, so a body naming a global failed to evaluate. The read is now a read, and the checker seeds the merge substitution from the program's own top-level actions, as it already does for a fact. `(get-size! "<name>")` also skips reserved tables now, as the no-argument form already did, and `(print-function <global> n)` respects `n`.
- **A global bound to a primitive works under the term/proof encoding.** It was rejected outright ("for silly internal reasons"). The encoding treated every global as holding an e-class, so it built the congruence view and emitted a union into a union-find the sort does not have; and a fact reading one was justified by the view's proof, whose term names the global's slot as well as its value and so lines up with nothing else in the proof. A global is now asked whether its value is, or contains, an e-class: one that does keeps the constructor-like encoding, one that does not takes the ordinary `:no-merge` route, is read by a new value-column lookup that fails rather than defaulting, and is justified by its value — giving the same proof the fact would have with the value written literally. `before-proofs`, `prims`, `repro-unsound` and `luminal-llama` now run under proof testing. A global bound to a *container* is still rejected, and now says why: its elements go stale as the e-graph merges and a global's own row has no rebuild.
- **Every global of a sort shares one table.** `remove_globals` gave each global its own nullary function, so the term/proof encoding gave each one a term relation, an FD view, a rebuild index and a rebuild rule; 1000 global `let`s expanded to 9049 encoded commands and 3009 installed rules, against 53 and 10 for 1000 local `let`s in one rule. A global is now one row of `@Globals_<Sort>`, keyed by a slot id, so that schema is emitted once per sort. The table is named by the fresh generator, so no program can declare a function that collides with it — and because it is reserved, neither `(get-size!)` nor `(get-size! "<global>")` counts a `let` any more, matching `print-size`, which already skipped globals. Reading a global now probes a keyed row where it used to read a nullary function, which costs a few percent in a rule that reads one every iteration and saves rather more where a program binds many (3000 base-sort `let`s: 0.107 s to 0.050 s). Doing it in the desugaring rather than in the encoder means the term encoding collapses the same way. On `luminal-llama` this is 8.14 s to 6.42 s wall under `--proofs` and 4.58 s to 3.19 s under `--term-encoding` (`bench.py`, min of 6). A program that binds few globals pays the read side without collecting that win: `eggcc-2mm-pass1` is 3% slower under `--term-encoding`. Proof output is unchanged (every proof snapshot is byte-identical); proof extraction skips a global's row so a proof still names the term the global was bound to, and serialization and `(print-function <global>)` both read the global's own name back from the row's key rather than showing the shared table.
