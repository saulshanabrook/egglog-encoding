...
- In the term/proof encoding, cache each constructor application built in an action scope and reuse its e-class for any repeat instead of minting a fresh id and re-interning it (dropping the redundant `get-fresh!`, term-relation row, and `set-if-empty`). This covers two cases: a head build of an application the body already matched reuses the body view read's e-class, and a repeated build of the same application in one scope (common subexpression, e.g. `(Foo (Bar x) (Bar x))`) reuses the first build. The match is keyed on the instrumented children, so it composes through nested terms. In proof mode a body-match reuse still builds the *natural* node (a parent's `Congr` and the rule-head check need its as-built shape) but skips the view dedup, reusing the body view read's proof `eclass = f(children)`; a common-subexpression reuse shares the first build's natural node across the repeats.
