...
- External functions can declare the tables they read and write (`egglog_bridge::EGraph::declare_external_func_table_deps`); a `:merge` body calling one contributes those to the calling table's dependency-graph entry, so it is stratified above its read targets. This lets the term/proof encoding hash-cons node tables inside `:merge` bodies.
