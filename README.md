...
| --- | --- | --- |
| Math | The paper artifact's exact Rational source and seven seeds, sampled with its seminaive backoff policy; committed wrappers provide sparse checkpoints at 0, 10, ..., 100 iterations | A source equality is checked and all 13 source table sizes are printed for comparison with artifact totals |
...
scheduler contract, and artifact table-size oracles. Checkpoints above ten are
opt-in scaling cases and can exhaust memory before reaching the timeout,
especially with proof instrumentation.
...
suite; the standalone paper harness owns end-to-end artifact reproduction.

### Standalone paper harness

`paper_bench.py` reproduces the historical artifact separately from the normal
proof-overhead benchmark cache. Set up a fresh authenticated cache with:

```bash
./paper_bench.py setup \
  --url 'https://zenodo.org/records/7709794/files/artifact.tar.gz?download=1'
```

The setup command requires a nonexistent destination, verifies the complete
archive against the pinned SHA-256, extracts only allowlisted regular files,
and records a deterministic inventory. Once set up, this command re-verifies
the cache without downloading anything:

```bash
./paper_bench.py setup
```

Run the bounded production lanes independently:

```bash
./paper_bench.py run quick math
./paper_bench.py run quick pointer
./paper_bench.py run quick herbie
```

The presets are intentionally evaluation-specific:

| Preset | Math | Pointer | Herbie |
| --- | --- | --- | --- |
| `quick` | Checkpoint 10, one observation per historical/current lane | `initdb`, one observation per lane | One end-to-end `2cos.fpcore` observation |
| `representative` | Checkpoint 10, five observations per lane | `initdb`, three observations per lane | Unsupported; fails explicitly |
| `artifact-full` | Checkpoints 0 through 100 by tens, one observation per lane | `initdb`, five observations per lane | Unsupported; fails explicitly |

Math compares the artifact's Egg and Eqlog engines with current off, term, and
proof modes. Historical Math observations invoke the staged Rust binary
directly, run only the selected checkpoint, and disable its separate naive-Eqlog
pass. Current observations likewise invoke the Egglog binary directly, so both
wall measurements use a complete engine-process boundary rather than including
a Python child-process wrapper on only one side. The harness parses the one-row
historical CSV output and requires its exact benchmark, engine, and size; current
lanes must print the exact ordered 13-table oracle. Current proof lanes run their
timed `--proofs` observation before an untimed strict `--proof-testing`
validation, so the correctness gate cannot warm the timed sample. Pointer
requires exact complete output lines in every lane.

The Herbie lane is a historical end-to-end smoke test, not a proof-overhead
comparison. It runs one FPCore problem through Herbie, Racket FFI, artifact
Eqlog, and extraction, then checks the parsed status and error improvement. A
current-Egglog treatment lane would require porting the 2023 FFI integration to
the current API. Broader Herbie presets are rejected rather than relabeling the
one-problem smoke test. Its observation intentionally measures a Python
validation wrapper around the Racket child, so its wall time must not be mixed
with the direct-process Math or pointer comparisons.

Historical Rust sources remain read-only in `.paper-artifact/`. Build hooks copy
them into `.paper-build/` and apply a fail-closed compatibility patch:

- qualify four `ast::Rule` references exposed by current Rust name resolution;
- add `--only-iter` and `--disable-eqlog-naive` to the Math driver; and
- make nested copied crates standalone Cargo workspaces.

The historical compatibility builds use Rust 1.91.0 and record that fact; they
are not claimed to reproduce the paper's compiler binary. Every Cargo build
disables machine-global `RUSTC_WRAPPER` configuration to avoid unrecorded cache
behavior. The bounded Herbie build additionally uses the checked-in lockfile at
`paper_benchmarking/assets/herbie/Cargo.lock` and checksum-pinned Racket package
revisions. Each invocation recreates its adapter-owned Racket home, verifies the
installed revisions and source links, and records pre-observation hashes for the
Racket executable, complete Racket home, and native `egg_math` library. Generated
state from an earlier invocation is never reused.

Each invocation creates an exclusive `.paper-results/<run-id>/` directory:

| File | Contents |
| --- | --- |
| `manifest.json` | Immutable selection, commands, environments, inputs, versions, repository state, and machine context |
| `runs.jsonl` | Durable process outcomes, primary executable hash, and declared nested-runtime hashes in execution order |
| `logs/` | Complete per-process stdout and stderr |
| `summary.md` | The exact Markdown printed to stdout |
| `status.json` | Current or terminal run lifecycle state |

Build/progress diagnostics use stderr. Failed output gates, timeouts, and
infrastructure errors remain explicit statuses and never become successful
timing samples. This harness never reads or writes `.reports.jsonl`.
