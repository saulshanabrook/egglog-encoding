...
def copy_and_sha256(source: IO[bytes], destination: IO[bytes]) -> tuple[str, int]:
...
def hash_path(path: Path) -> PathDigest:
    ...
    if stat.S_ISLNK(metadata.st_mode):
        ...
        digest = hashlib.sha256(b"paper-symlink-v1\0" + os.fsencode(target)).hexdigest()
        return PathDigest(str(absolute_path), "symlink", digest, None)
...
def _sha256_directory(root: Path) -> str:
    ...
    for child in children:
        ...
        if stat.S_ISLNK(metadata.st_mode):
            _update_tree_digest(digest, b"L", relative, os.fsencode(os.readlink(child)))
