...
The accepted outputs used the names `parameter-forward.json`,
`parameter-reverse.json`, `propel-gset-forward.json`,
`propel-gset-reverse.json`, `propel-medium-forward.json`, and
`propel-medium-reverse.json`; the four diagnostics used the corresponding
`parameter-*-timing.json` names. Raw JSON is no longer tracked. Its hashes are
preserved in [`../RAW_RESULT_HASHES.md`](../RAW_RESULT_HASHES.md).
