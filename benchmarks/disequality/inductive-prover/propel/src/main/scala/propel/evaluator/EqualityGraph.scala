...
private case class EgglogEqualityGraph(
  ...
  override def compare(lhs: EClass, rhs: EClass): GraphComparison =
    runtime.checkEqual(terms(lhs.id), terms(rhs.id)) match
      case EgglogCommandResult.Success(_) => GraphComparison.Equal
      case EgglogCommandResult.CheckFailed(_) =>
        runtime.checkKnownDisequal(terms(lhs.id), terms(rhs.id)) match
          case EgglogCommandResult.Success(_) => GraphComparison.Unequal
          case EgglogCommandResult.ExpectFailFailed(_) => GraphComparison.Indeterminate
  ...
  override def rebuild(): Unit =
    runtime.rebuild() match
      case EgglogCommandResult.Success(_) => ()
      case EgglogCommandResult.Error(message) => throw RuntimeException(message)
  ...
  override def hasContradiction: Boolean =
    runtime.checkConsistency() match
      case EgglogCommandResult.Success(_) => true
      case EgglogCommandResult.ExpectFailFailed(_) => false
      case EgglogCommandResult.Error(message) => throw RuntimeException(message)
