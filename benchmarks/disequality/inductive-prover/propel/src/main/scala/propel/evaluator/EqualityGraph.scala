...
object EqualityGraphBackend:
  ...
  private case class EgglogBackend(
    ...
    override def empty: EqualityGraph = EgglogEqualityGraph(
      template.newRuntime(recordInteractions),
      mutable.Map.empty,
    )
...
private[evaluator] case class EgglogEqualityGraph(
  ...
  override def compare(lhs: EClass, rhs: EClass): GraphComparison =
    runtime.checkEqual(terms(lhs.id), terms(rhs.id)) match
      case EgglogCommandResult.Success => GraphComparison.Equal
      case EgglogCommandResult.CheckFailed =>
        runtime.checkKnownDisequal(terms(lhs.id), terms(rhs.id)) match
          case EgglogCommandResult.Success => GraphComparison.Unequal
          case EgglogCommandResult.ExpectFailFailed => GraphComparison.Indeterminate
          case EgglogCommandResult.Error(message) => throw RuntimeException(message)
          case result => throw IllegalStateException(s"unexpected egglog disequality result: $result")
      case EgglogCommandResult.Error(message) => throw RuntimeException(message)
      case result => throw IllegalStateException(s"unexpected egglog equality result: $result")
  override def rebuild(): Unit =
    runtime.rebuild() match
      case EgglogCommandResult.Success => ()
      case EgglogCommandResult.Error(message) => throw RuntimeException(message)
      case result => throw IllegalStateException(s"unexpected egglog rebuild result: $result")
  override def hasContradiction: Boolean =
    runtime.checkConsistency() match
      case EgglogCommandResult.Success => true
      case EgglogCommandResult.ExpectFailFailed => false
      case EgglogCommandResult.Error(message) => throw RuntimeException(message)
      case result => throw IllegalStateException(s"unexpected egglog consistency result: $result")
  override def stats: EqualityGraphStats = EqualityGraphStats(runtime.numClasses, runtime.numNodes)
  override def writeSnapshot(sourcePath: String, desugaredPath: String): Unit =
    Files.writeString(Paths.get(sourcePath), runtime.source)
    Files.writeString(Paths.get(desugaredPath), runtime.desugaredSource)
