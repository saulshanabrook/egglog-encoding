...
private object EgglogApi:
  ...
  def egglog_disequality_flush(graph: Ptr[Byte]): Ptr[Byte] = extern
  def egglog_disequality_rebuild(graph: Ptr[Byte]): Ptr[Byte] = extern
  def egglog_disequality_check_equal(
      graph: Ptr[Byte],
      lhs: CUnsignedLongLong,
      rhs: CUnsignedLongLong,
  ): Ptr[Byte] = extern
  def egglog_disequality_check_known_disequal(
      graph: Ptr[Byte],
      lhs: CUnsignedLongLong,
      rhs: CUnsignedLongLong,
  ): Ptr[Byte] = extern
  def egglog_disequality_check_consistency(graph: Ptr[Byte]): Ptr[Byte] = extern
  def egglog_command_result_status(result: Ptr[Byte]): CInt = extern
  def egglog_command_result_output_count(result: Ptr[Byte]): CSize = extern
  def egglog_command_result_output_kind(result: Ptr[Byte], index: CSize): CInt = extern
  def egglog_command_result_output_text(result: Ptr[Byte], index: CSize): CString = extern
  def egglog_command_result_error(result: Ptr[Byte]): CString = extern
  def egglog_command_result_free(result: Ptr[Byte]): Unit = extern
  ...
  def egglog_disequality_source(graph: Ptr[Byte]): Ptr[Byte] = extern
  def egglog_disequality_desugared_source(graph: Ptr[Byte]): Ptr[Byte] = extern
  def egglog_owned_string_data(value: Ptr[Byte]): CString = extern
  def egglog_owned_string_len(value: Ptr[Byte]): CSize = extern
  def egglog_owned_string_free(value: Ptr[Byte]): Unit = extern
...
private final class NativeEgglogRuntime private (
  ...
  private def command(result: Ptr[Byte], operation: String): EgglogCommandResult =
    if result.toLong == 0 then fail(operation)
    try
      val outputs = Vector.tabulate(EgglogApi.egglog_command_result_output_count(result).toInt) { index =>
        val text = EgglogApi.egglog_command_result_output_text(result, index.toUSize)
        if text.toLong == 0 then
          throw RuntimeException(s"failed to $operation: command output $index had no text")
        val kind = EgglogApi.egglog_command_result_output_kind(result, index.toUSize) match
          case 0 => EgglogCommandOutputKind.PrintFunctionSize
          case 1 => EgglogCommandOutputKind.PrintAllFunctionsSize
          case 2 => EgglogCommandOutputKind.ExtractBest
          case 3 => EgglogCommandOutputKind.ExtractVariants
          case 4 => EgglogCommandOutputKind.ProveExists
          case 5 => EgglogCommandOutputKind.OverallStatistics
          case 6 => EgglogCommandOutputKind.PrintFunction
          case 7 => EgglogCommandOutputKind.RunSchedule
          case 8 => EgglogCommandOutputKind.UserDefined
          case value => throw RuntimeException(s"failed to $operation: unknown command output kind $value")
        EgglogCommandOutput(kind, fromCString(text))
      }
      EgglogApi.egglog_command_result_status(result) match
        case 0 => EgglogCommandResult.Success(outputs)
        case 1 => EgglogCommandResult.CheckFailed(outputs)
        case 2 => EgglogCommandResult.ExpectFailFailed(outputs)
        case 3 =>
          val message = EgglogApi.egglog_command_result_error(result)
          EgglogCommandResult.Error(
            if message.toLong == 0 then s"failed to $operation: unknown Rust backend error"
            else s"failed to $operation: ${fromCString(message)}",
          )
        case value => EgglogCommandResult.Error(s"failed to $operation: unknown command status $value")
    finally EgglogApi.egglog_command_result_free(result)

  private def ownedString(result: Ptr[Byte], operation: String): String =
    if result.toLong == 0 then fail(operation)
    try
      val data = EgglogApi.egglog_owned_string_data(result)
      if data.toLong == 0 then throw RuntimeException(s"failed to $operation: result had no data")
      val value = fromCString(data)
      val expectedBytes = EgglogApi.egglog_owned_string_len(result).toLong
      if value.getBytes("UTF-8").nn.length != expectedBytes then
        throw RuntimeException(s"failed to $operation: invalid UTF-8 byte length")
      value
    finally EgglogApi.egglog_owned_string_free(result)
  ...
  override def flush(): EgglogCommandResult =
    command(EgglogApi.egglog_disequality_flush(graph), "flush e-graph actions")

  override def rebuild(): EgglogCommandResult =
    command(EgglogApi.egglog_disequality_rebuild(graph), "rebuild the e-graph")

  override def checkEqual(lhs: Long, rhs: Long): EgglogCommandResult =
    command(
      EgglogApi.egglog_disequality_check_equal(graph, lhs.toULong, rhs.toULong),
      "check e-class equality",
    )

  override def checkKnownDisequal(lhs: Long, rhs: Long): EgglogCommandResult =
    command(
      EgglogApi.egglog_disequality_check_known_disequal(graph, lhs.toULong, rhs.toULong),
      "check known e-class disequality",
    )

  override def checkConsistency(): EgglogCommandResult =
    command(EgglogApi.egglog_disequality_check_consistency(graph), "check graph consistency")
  ...
  override def source: String =
    ownedString(EgglogApi.egglog_disequality_source(graph), "render egglog source")

  override def desugaredSource: String =
    ownedString(EgglogApi.egglog_disequality_desugared_source(graph), "render desugared egglog source")
