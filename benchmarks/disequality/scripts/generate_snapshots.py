...
EUF_HEADER = (
    ...
    b";; MiniSat candidate assignment produced by the Dis/Equality Graphs EUF artifact integration,\n"
...
def validate_direct_snapshot(contents: bytes, sort_name: str, description: str) -> None:
    ...
    forbidden = ("BenchmarkNode", "BenchmarkTerms", "HostTermAt", "HostWitness", "vec-of")
...
def replay(
    ...
    *,
    ...
    if process.returncode != 0:
        raise RuntimeError(f"replaying {path} with {encoding} failed:\n{process.stdout}")


def replay_snapshots(binary: Path, core_binary: Path, output: Path) -> int:
    replay_count = 0
    with tempfile.TemporaryDirectory(prefix="egglog-parameter-facts-") as temporary:
        ...
        for path in sorted(output.glob("*.egg")):
            ...
            for encoding in ENCODINGS:
                replay(binary, encoding, path, facts, pass_encoding=True)
                replay_count += 1
        ...
        for path in sorted((output / "snapshots").glob("*.desugared.egg")):
            ...
            replay(core_binary, encoding, path, facts, pass_encoding=False)
            replay_count += 1
    return replay_count
...
def generate(args: argparse.Namespace) -> dict[Path, bytes]:
    ...
    with tempfile.TemporaryDirectory(prefix="egglog-disequality-snapshots-") as temporary:
        ...
        for encoding in ENCODINGS:
            ...
            index = (
                max(range(len(raw)), key=lambda candidate: raw[candidate].stat().st_size)
                if args.propel_graph_index < 0
                else args.propel_graph_index
            )
            if index >= len(raw):
            ...
            if args.propel_graph_index < 0 and b"check-known-disequal" not in raw_source:
                raise RuntimeError(
                    f"Propel graph {index} does not exercise the pair-disequality query",
                )
            propel_sources.append(raw_source)
            host_desugared[("propel-gset-comm", encoding)] = desugared[index].read_bytes()
        if selected_index is None:
            raise RuntimeError("Propel did not emit a selectable graph")
        ...
        propel_header = (
            ...
            ";; imported from the Dis/Equality Graphs artifact. This is zero-based graph\n"
            f";; index {selected_index} (position {selected_index + 1} of {graph_counts[0]}), rendered with\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--core-egglog-binary",
        type=Path,
        default=root / "target" / "debug" / "egglog",
    )
    parser.add_argument(
        "--propel-input",
        type=Path,
    ...
    replay_binaries = (args.egglog_binary, args.core_egglog_binary)
    required_binaries = (
        replay_binaries
        if args.replay
        else (
    ...
    if args.replay:
        ...
        replay_count = replay_snapshots(
            args.egglog_binary.resolve(),
            args.core_egglog_binary.resolve(),
            args.output,
        )
