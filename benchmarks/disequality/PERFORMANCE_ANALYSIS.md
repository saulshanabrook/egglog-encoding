...
Run dates: 2026-08-12 through 2026-09-10
...
- Rust 1.91.0 and Cargo 1.91.0; the older case-study tranches used uv 0.12.3,
  ...
  follow-up used uv 0.12.6; the final AST-first gate used uv 0.12.12 and
  Hyperfine 1.20.0
- final AST-first gate from clean measured commit `069d9ac45d9da281ba4eb1803a39b0ad27d6357a`
  against clean merged baseline `4737838d853f988ae550a64af8f0c9621e7584ef`;
  both include `origin/main` at `82aab2a5ea4e37a46310a5ee97e02fec156fcc77`
...
- final AST-first gate in two opposite endpoint orders: eight samples per
  order for small Propel, four for medium Propel, five for `uf.815405`, and
  three for `uf.614981`; the recording/export ablation uses eight samples per
  order
...
| --- | --- | ---: | --- |
| Final AST-first, Propel small | candidate / merged baseline | +1.27% to +4.79% mean | actual AST actions and structured query outcomes remain below the 10%-in-both-orders gate |
| Final AST-first, Propel medium | candidate / merged baseline | +3.68% to +5.42% mean | AST history is not retained in ordinary runs; graph/database work remains |
| Final AST-first, published EUF pair | candidate / merged baseline | -5.80% to +3.04% mean | no material regression on 245- and 627-model inputs |
| Final opt-in capture, Propel small | recording+export / recording off | +16.06% to +18.25% mean | includes tracing, rendering, desugaring, and 104 file writes |
...
| Earlier typed-host AST, Propel small | candidate / source-reparse parent | 0.967x DE; 0.983x NEE wall | historical isolated change before the final merge and trace semantics |
| Earlier typed-host AST, Propel medium | candidate / source-reparse parent | 0.968x DE; 0.960x NEE wall | historical isolated change before the final merge and trace semantics |
...
## Final AST-first host integration gate

The final 2026-09-10 measurement compares the clean merged branch immediately
before this refactor (`4737838d`) with the clean AST-first implementation
(`069d9ac4`). Both endpoints therefore include the same `origin/main` merge.
The candidate stores pending mutations as actual `egglog::ast::Action` values,
submits a single `Command::Actions` per flush, and interprets structured query
results from `EGraph::run_program`. Neither endpoint writes captures during the
ordinary A/B.

Each workload was measured in forward and exact reverse endpoint order. The
table reports arithmetic means and candidate deltas for each order:

| Workload | Encoding | Forward baseline -> candidate | Reverse baseline -> candidate |
| --- | --- | ---: | ---: |
| Propel `gset_comm` | DE | 194.9 -> 202.8 ms (+4.05%) | 194.5 -> 203.9 ms (+4.79%) |
| Propel `gset_comm` | NEE | 184.3 -> 189.1 ms (+2.61%) | 190.1 -> 192.5 ms (+1.27%) |
| Propel `tip_bin_plus_assoc` | DE | 4.480 -> 4.645 s (+3.68%) | 4.392 -> 4.630 s (+5.42%) |
| Propel `tip_bin_plus_assoc` | NEE | 3.607 -> 3.793 s (+5.18%) | 3.610 -> 3.796 s (+5.16%) |
| EUF `uf.815405` | DE | 893.6 -> 911.2 ms (+1.97%) | 903.4 -> 916.9 ms (+1.50%) |
| EUF `uf.815405` | NEE | 509.1 -> 524.6 ms (+3.04%) | 528.1 -> 524.6 ms (-0.66%) |
| EUF `uf.614981` | DE | 8.621 -> 8.533 s (-1.03%) | 9.059 -> 8.534 s (-5.80%) |
| EUF `uf.614981` | NEE | 4.860 -> 4.997 s (+2.82%) | 4.881 -> 5.024 s (+2.94%) |

The predeclared landing blocker was a slowdown above 10% in both orders. The
largest directional slowdown is 5.42%, so this gate passes. Several reversed
baseline arms contain visible high outliers; the table therefore does not claim
that negative deltas are speedups or that small deltas are precise estimates.

The source-export ablation is separate. On candidate NEE `gset_comm`, enabling
`--emit-source-dir` adds 16.06% in forward order and 18.25% in reverse order.
That path creates the persistent trace, renders ASTs, desugars the replay, and
overwrites 104 files. It does not measure ordinary AST execution, because
ordinary graphs do not retain a replay.

[`reports/ast-first-host/README.md`](reports/ast-first-host/README.md) records
the exact revisions, executable and input hashes, command matrix, sample
counts, raw-output hashes, and limitations. The raw Hyperfine JSON remains in
`/tmp` rather than Git.
...
The report directory retains exact commands, executable/input hashes, and the
summary. Raw JSON is no longer tracked; its historical hashes are in
[`reports/RAW_RESULT_HASHES.md`](reports/RAW_RESULT_HASHES.md).
...
The report directory retains exact commands, source revisions,
executable/input hashes, environment, and the summary. Raw JSON is no longer
tracked; its historical hashes are in
[`reports/RAW_RESULT_HASHES.md`](reports/RAW_RESULT_HASHES.md).
...
three samples per order; small Propel uses five. Every sample was accepted.
...
Exact commands, executable and input hashes, environment, and summaries remain
in [`reports/set-de-follow-up/`](reports/set-de-follow-up/). Raw JSON is no
longer tracked; its historical hashes are in
[`reports/RAW_RESULT_HASHES.md`](reports/RAW_RESULT_HASHES.md). The omitted published
...
Run provenance remains under
[`reports/term-language-performance/`](reports/term-language-performance/),
and historical raw-output hashes are in
[`reports/RAW_RESULT_HASHES.md`](reports/RAW_RESULT_HASHES.md).
