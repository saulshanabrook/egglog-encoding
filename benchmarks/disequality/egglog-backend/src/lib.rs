...
/// The classified status of an egglog command.
#[derive(Clone, Debug)]
...
fn desugar_source(encoding: DisequalityEncoding, source: &str) -> Result<String, BackendError> {
...
fn ffi_call<T>(
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: Every exported operation requires a live pointer returned by
        // `egglog_disequality_graph_new_from_template` or `_clone`.
...
mod tests {
    ...
    fn pending_actions_are_ast_and_record_the_submitted_batch() {
        ...
        let result = graph.flush_unlocked().unwrap();
    ...
    fn c_abi_exposes_structured_query_statuses_and_owned_source() {
        ...
        unsafe {
            let template = egglog_disequality_template_new(2, 0, ptr::null());
            assert!(!template.is_null());
            assert_eq!(egglog_disequality_template_finish(template), 0);
            let graph = egglog_disequality_graph_new_from_template(template, 1);
            egglog_disequality_template_free(template);
    ...
    #[test]
    fn c_abi_contains_errors_and_panics() {
        // SAFETY: Every pointer in this test is created and freed exactly once
        // through this module's C ABI.
        unsafe {
            let template = egglog_disequality_template_new(0, 0, ptr::null());
            assert!(!template.is_null());
            assert_eq!(egglog_disequality_template_finish(template), 0);
            let graph = egglog_disequality_graph_new_from_template(template, 0);
            egglog_disequality_template_free(template);
            assert!(!graph.is_null());
